...
namespace HAL
{
    ...
    public:
        ...
        /**
         * @brief [cardputer-deck addition] Quick, bounded presence check.
         * Does SPI init + reset + a version-register read only — skips the full
         * init() config sequence, which would block on ~1s BUSY timeouts per
         * command when no chip is attached. Safe to call at boot to detect the
         * radio without stalling. Returns true if a valid SX1262 responds.
         * @param out_version optional: receives the version byte (0x00/0x01)
         */
        bool probe(uint8_t* out_version = nullptr);
